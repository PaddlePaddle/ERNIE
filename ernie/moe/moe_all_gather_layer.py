...
class AlltoAllSmart(paddle.autograd.PyLayer):
    """
    Perform dispatch inputs alltoall. (GPU VERSION)
    """
...
class AlltoAllSmartXPU(paddle.autograd.PyLayer):
    ...
    def backward(
        ...
        in_tensor_list_bw = [paddle.concat(tensors, 0) if tensors else dummy_input for tensors in in_tensors_by_rank_bw]
...
# Conditionally select the XPU AlltoAllSmart implementation
        ...
        if not self.use_expert_out_alltoall:
            ...
            expert_out_to_combine, router_loss2, distributed_input_to_alltoall_out = AlltoAllSmart.apply(
                *dispatched_input,
                ...
                capacity=dispatched_input[0].shape[1] if self.use_padding and dispatched_input[0] is not None else None,
