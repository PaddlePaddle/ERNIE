    ...
    def _convert_example_to_record(self, example, max_seq_length, tokenizer, index):
        ...
        if self.add_header_eos_token:
            system_header_tokens = (
                [self.header_start_token]
                + tokenizer.tokenize("system")
                + [self.header_end_token]
            ...
            assisant_header_tokens = (
                [self.header_start_token]
                + tokenizer.tokenize("assisant")
                + [self.header_end_token]
            )  # ['<mask:6>', 'assis', 'ant', '<mask:7>']
            knowledge_header_tokens = (
                [self.header_start_token]
                + tokenizer.tokenize("knowledge")
                + [self.header_end_token]
            )
        ...
        if self.use_agent_sys2:  # use_agent_sys2
            while turn_index >= 0:
                ...
                tmp_loss_mask = (
                    [0] * (len(tokens_src) - 1)
                    + [example.label[turn_index]] * len(tokens_target)
                    + [0]
                )
            ...
            while turn_index >= 0:
                ...
                tokens_src, tokens_target = tokenizer.tokenize(src), tokenizer.tokenize(
                    tgt
                )
                if self.add_header_eos_token:
                    tokens_src = user_header_tokens + tokens_src + [self.end_token]
                    tokens_target = (
                        assisant_header_tokens + tokens_target + [self.end_token]
                    )
                ...
                if self.add_break_token_multi_turn_for_nontrigger_data:
                    if self.add_start_token:  # for eb35
                        ...
                        if self.add_header_eos_token:
                            ...
                            ) + loss_mask  # 1 for  break_turn_token
                        else:
        ...
        if skip_align_flag:
            ...
            def compute_s_by_policy(
                skip_distance, i, turns, skip_align_s_policy="default"
            ):
            ...
            if self.follow_deepseek:
                ...
                break_turn_token_ids = [
                    index
                    for index, token in enumerate(tokens)
                    if token == self.end_of_response
                ]
            else:
                break_token_ids = [
                    index
                    for index, token in enumerate(tokens)
                    if token == self.break_token
                ]
                break_turn_token_ids = [
                    index
                    for index, token in enumerate(tokens)
                    if token == self.break_turn_token
                ]
            ...
            for i in range(turns):
                ...
                s = compute_s_by_policy(
                    max_skip_distance, i, turns, self.skip_align_s_policy
                )
                if (
                    self.global_rng.random() < self.skip_align_sub_sample_prob
                    and max_skip_distance != 1
                ):
                    u += s
                # X    YX    Y
                if self.skip_align_method == "skip_inner":
                    pos_ids[
                        break_token_ids[i] : break_turn_token_ids[i] + 1
                    ] += u  # sepY(i)cls
                    if i != turns - 1:
                        pos_ids[
                            break_turn_token_ids[i] + 1 : break_token_ids[i + 1]
                        ] += u  # X(i+1)
                # XY    XY
                elif self.skip_align_method == "skip_outter":
                    if i != turns - 1:
                        pos_ids[
                            break_turn_token_ids[i]
                            + 1 : break_turn_token_ids[i + 1]
                            + 1
                ...
                # X    Y    X    Y
                    ...
                    max_skip_distance = self.skip_align_target_length - len(tokens) - u
                    max_skip_distance = (
                        max_skip_distance if max_skip_distance != 0 else 1
                    )
                    s = compute_s_by_policy(
                        max_skip_distance, i, turns, self.skip_align_s_policy
                    )
        ...
        if self.use_agent_sys2:
            ...
            if self.follow_deepseek:
                ...
                labels = [
                    label if label != self.break_turn_token else self.end_token
                    for label in labels
                ]
