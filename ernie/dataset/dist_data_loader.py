...
class DistDataLoader(paddle.io.DataLoader):
    ...
    def check_to_return_is_good(self, to_return):
        ...
        images = to_return.get("images", None)
        if images is not None:
            images = to_return["images"].cpu().numpy()
        token_type_ids = to_return["token_type_ids"].cpu().numpy().squeeze(0)
        inbatch_pack_offset = to_return["inbatch_pack_offset"].cpu().numpy().squeeze(0)
        grid_thw = to_return.get("grid_thw", None)
        if grid_thw is not None:
            grid_thw = to_return["grid_thw"].cpu().numpy()
        position_ids = to_return["position_ids"].cpu().numpy().squeeze(0)
        use_rope_3d = False
            ...
            for start, end in zip(
                ...
                if image_num > 0:
                    ...
                    if ((part_grid_thw == 0).all(axis=-1)).any():
                        ...
                        with open("Error-log.jsonl", "a") as f:
                            f.write(
                                json.dumps(
                                    {
                                        ...
                                        "inbatch_pack_offset": inbatch_pack_offset.tolist(),
