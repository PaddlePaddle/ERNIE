...
- 我们提供了[藏语训练数据集](https://paddleformers.bj.bcebos.com/datasets/ocr_vl_sft-train_bo.jsonl)可用于微调 PaddleOCR-VL-0.9B 对藏语进行识别，或者你可以按照 [SFT VL 数据集格式](./datasets.md#sft-vl-dataset) 来构建自己的微调数据集。
...
```
- 我们提供了[藏语评估数据集](https://paddleformers.bj.bcebos.com/datasets/ocr_vl_sft-eval_bo.jsonl)，可以使用 [PaddleX](https://github.com/PaddlePaddle/PaddleX) 进行推理来观察微调效果。
