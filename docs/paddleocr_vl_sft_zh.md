...
虽然 PaddleOCR-VL 在常见场景下表现出色，但在许多特定或复杂的业务场景中，其性能会遇到瓶颈。例如：
- 特定行业与专业领域
...
- 在 [huggingface](https://huggingface.co/PaddlePaddle/PaddleOCR-VL/tree/main/PaddleOCR-VL-0.9B) 或者 [modelscope](https://modelscope.cn/models/PaddlePaddle/PaddleOCR-VL/files) 可以下载模型。
- 我们提供了[藏语训练数据集](https://paddleformers.bj.bcebos.com/datasets/ocr_vl_sft-train_bo.jsonl)可用于微调 Paddle OCR 对藏语进行识别，或者你可以按照 [SFT VL 数据集格式](./datasets.md#sft-vl-dataset) 来构建自己的微调数据集。
...
```
- 我们提供了[藏语评估数据集](https://paddleformers.bj.bcebos.com/datasets/ocr_vl_sft-eval_bo.jsonl)，可以使用 [PaddleX](https://github.com/PaddlePaddle/PaddleX) 进行推理。
