...
from finetune.sequence_label import create_model, evaluate, predict, chunk_eval, calculate_f1
        ...
        while True:
            try:
                ...
                if steps % args.skip_steps != 0:
                    ...
                    out = train_exe.run(fetch_list=fetch_list)[:4]
                    if "learning_rate" in graph_vars:
                        np_labels, np_probs, np_lens, np_loss, np_lr = out
                        lr = float(np_lr[0])
                    else:
                        np_labels, np_probs, np_lens, np_loss = out
                    loss = np_loss.mean()
                    num_label, num_infer, num_correct = chunk_eval(
                        np_labels, np_probs, np_lens, args.num_labels, dev_count)
