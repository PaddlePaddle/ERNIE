...
class InputIdsMassageArguments:
    ...
    use_loc_specialtoken: bool = field(
        default=False, metadata={"help": "location use special token"}
    )
    use_crop_specialtoken: bool = field(
        default=False, metadata={"help": "crop use special token"}
    )
    use_pic_id: bool = field(default=True, metadata={"help": "add Picture Id"})

    augment_conf: str = field(
        default="conf/config_transform.yaml",
        metadata={"help": "image augment conf path"},
    )
    ...
    use_multi_image_crop: bool = field(
        default=False, metadata={"help": "use multi image crop"}
    )
    ...
    def __post_init__(self):
        self.crop_tile_option = [
            int(op) for op in self.crop_tile_option.strip().split(",")
        ]
        self.crop_tile_rate = [
            float(op) for op in self.crop_tile_rate.strip().split(",")
        ]
        ...
        assert self.video_fix_crop in [
            -1,
            2,
        ], "video_fix_crop only supported values are -1 and 2"
