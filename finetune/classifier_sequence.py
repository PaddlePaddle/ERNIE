...
"""Model for classifier via sequence output."""
...
def create_model(args,
    ...
    if is_classify:
        ...
        pyreader = fluid.layers.py_reader(
            ...
            shapes=[[-1, args.max_seq_len, 1], [-1, args.max_seq_len, 1],
                    ...
                    [-1, args.max_seq_len, 1], [-1, 1], [-1, 1], [-1, 1]],
            dtypes=[
                'int64', 'int64', 'int64', 'int64', 'float32', 'int64', 'float32', 'int64'
            ],
            lod_levels=[0, 0, 0, 0, 0, 0, 0, 0],
            name=task_name + "_" + pyreader_name,
            use_double_buffer=True)

    (src_ids, sent_ids, pos_ids, task_ids, input_mask, seq_lens,
    ...
    unpad_enc_out = fluid.layers.dropout(
        x=unpad_enc_out,
        dropout_prob=0.1,
        dropout_implementation="upscale_in_train")
    fc0 = fluid.layers.fc(input=unpad_enc_out, size=hid_dim * 4)
    rfc0 = fluid.layers.fc(input=unpad_enc_out, size=hid_dim * 4)
