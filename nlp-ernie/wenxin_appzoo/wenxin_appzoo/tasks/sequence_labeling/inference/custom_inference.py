...
from wenxin.controller.inference import BaseInference
...
class CustomInference(BaseInference):
    ...
    def inference_batch(self):
        ...
        for batch_id, data in enumerate(dg()):
            data_ids, sample_entity_list = data
            ...
            # 回调给解析函数
            write_result_list = self.parser_handler(predict_results,
                    sample_list=sample_entity_list, params_dict=self.params, seq_lens=seq_lens)
            for write_item in write_result_list:
                output_file.write('\t'.join(write_item))
                output_file.write("\n")
