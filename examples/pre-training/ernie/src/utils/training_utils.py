...
def reset_per_device_batch_size(
    ...
    """
    根据`global_batch_size` 和 `per_device_train_batch_size` 调整`gradient_accumulation_steps` 和 `per_device_train_batch_size`。
    ...
    if batch_per_device < per_device_train_batch_size:
        ...
        ), f"global_bsz={global_batch_size} not evenly devided by world_size={dataset_world_size}, batch_per_device={batch_per_device}"
