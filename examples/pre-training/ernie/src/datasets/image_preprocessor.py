...
import numpy as np
import PIL
from paddleformers.transformers.image_processing_utils import (
    BaseImageProcessor,
    BatchFeature,
    get_size_dict,
)
...
__all__ = ["CLIPImageProcessor", "CLIPImageProcessorCupy"]


def numpy_to_rgb(image):
    """_summary_
...
class CLIPImageProcessor(BaseImageProcessor):
    ...
    def resize(
        ...
        **kwargs,
        ...
        if "shortest_edge" in size:
            ...
            assert "height" in size and "width" in size
            size = [size["height"], size["width"]]
        output_size = get_resize_output_image_size(
            image, size=size, default_to_square=False
        )
        return resize(
            image,
            size=output_size,
    ...
    def preprocess(
        ...
        **kwargs,
        ...
        images = [numpy_to_rgb(image) for image in images]
...
class CLIPImageProcessorCupy(BaseImageProcessor):
    r"""
    Constructs a CLIP image processor.
        ...
        **kwargs,
        ...
        if self.do_resize:
            if "shortest_edge" in self.size:
                self.size_new = self.size["shortest_edge"]
            else:
                assert "height" in self.size and "width" in self.size
                self.size_new = [self.size["height"], self.size["width"]]

        if self.do_normalize:
            # self.mean = np.array(self.image_mean, dtype=np.float32)
            # self.std = np.array(self.image_std, dtype=np.float32)
            self.mean = cp.array(self.image_mean, dtype=cp.float32)
            self.std = cp.array(self.image_std, dtype=cp.float32)
    ...
    def preprocess(
        ...
        **kwargs,
        ...
        # All transformations expect numpy arrays.
        # images = [to_numpy_array(image) for image in images]    # np.array(image)
        images = [np.array(image) for image in images]
        images = [numpy_to_rgb(image) for image in images]

        if do_resize:
            # PIL
            # images = [image.resize(self.size_new, resample=3, reducing_gap=None) for image in images]
            # cv2
            images = [
                cv2.resize(image, self.size_new, interpolation=cv2.INTER_CUBIC)
                for image in images
            ]

        # CuPy to cuda speed-up
        ...
        if do_rescale:
            images = [(image * rescale_factor).astype(cp.float32) for image in images]

        if do_normalize:
            images = [
                ((image - self.mean) / self.std).astype(cp.float32) for image in images
            ]

        # CuPy to Numpy
        images = [cp.asnumpy(image) for image in images]
