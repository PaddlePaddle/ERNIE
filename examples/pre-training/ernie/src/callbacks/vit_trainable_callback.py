...
class VitTrainableCallback(TrainerCallback):
    ...
    def __init__(self, args, model, auto_cast_func=None, patches_per_image=256):
        ...
        for n, p in self.vision_model.named_parameters():
            # need_clip 只会进行is_False判断，使用字符串增加额外信息，用于解决vit训练pp gradnorm算重复问题。
            p.need_clip = "pp_non_distributed"  # used to avoid duplicate gradnorm
        ...
        # hack extract_feature func 每个pp自行处理
        ...
        if args.pipeline_parallel_degree > 1:
            ...
            def _prepare_pipeline_inputs_func_wrapper(inner_self, data):
                def wrap(micro_data):
                    ...
                    ):  # TODO hard code image_features 在第4位
                        fea = inputs[2]
    ...
    def on_optimizer_begin(
        ...
        **kwargs,
        ...
        if args.pipeline_parallel_rank == 0:
            ...
            # logger.info(f"FEATURES_GRADS____{image_features_grad.shape}")
            # 在这里pp grad scatter之前把grad按照进vit的顺序对应弄好，后面直接scatter_varlen，减少send_recv通信
        ...
        # 提前gather grad的hidden_size维度，否则会影响冗余alltoall消除优化的性能
        gather_grads = []
        if self.mp_degree > 1:
            for grad in grads:
                if getattr(self.config.vision_config, "variable_resolution", False):
                    S, C = grad.shape
                    grad = grad.reshape([-1, C * self.config.spatial_conv_size**2])
                grad = all_gather(grad, axis=-1, group=self.mp_group)
                if getattr(self.config.vision_config, "variable_resolution", False):
                    grad = grad.reshape([S, -1])
            ...
            if self.mp_degree > 1:
                with paddle.no_grad():
                    ...
                    if shard_head:
                        seqlen = g.shape[0]
                        num_pad = (
                            math.ceil(seqlen / self.mp_group.nranks)
                            * self.mp_group.nranks
                            - seqlen
                        )
            ...
            for (im, thw), grad in zip(self.images_buffer, grads):
                ...
                if self.vit_second_fwd_batch_size:
                    ...
                    for i, g, t in zip(
                        ...
                        _inner_backward(i, t, g)
                else:
                    _inner_backward(im, thw, grad)
        ...
        with profile("vit__pp_reduce_grad"):
            ...
            if self.dp_group.nranks > 1:
                fused_allreduce_gradients_with_group(
                    self.vision_model.parameters(), self.dp_group
                )
