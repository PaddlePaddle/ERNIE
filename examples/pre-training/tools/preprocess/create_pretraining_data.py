...
def get_args():
    ...
    parser.add_argument(
        "--model_name_or_path", type=str, required=True, help="What model to use."
    )
    group = parser.add_argument_group(title="data input/output")
    group.add_argument(
        "--input_path", type=str, required=True, help="Path to input JSON files."
    )
    group.add_argument(
        "--output_prefix",
        type=str,
        ...
        for length in range(max_word_length, 0, -1):
            ...
            if "".join(tokens[i : i + length]) in words_set:
                ...
                for lens in range(1, length):
                    new_tokens.append("##" + tokens[i + lens])
                i += length
                has_add = True
                break
...
class Converter(object):
    ...
    def initializer(self):
        Converter.tokenizer = ErnieBotTokenizer.from_pretrained(
...
def main():
    ...
    sample_tokenizer = ErnieBotTokenizer.from_pretrained(args.model_name_or_path)
