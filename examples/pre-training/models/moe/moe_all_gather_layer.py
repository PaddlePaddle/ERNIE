...
from .token_dispatcher.token_dispatcher import MoEFlexTokenDispatcher
from paddleformers.utils.tools import get_env_device
from models.utils import get_global_training_logs, FakeGather, FusedUnpermutation
...
class DeepEPMOELayerMultiModal(MOEAllGatherLayerV2):
    """_summary_
    ...
    def __init__(
        ...
        super().__init__(
            ...
            k,
            enable_bpr,
            False,  # enable_reverse_token_drop
            all_to_all_dropout,
            group_experts,
            True,  # use_expert_out_alltoall
            False,  # use_expert_alltoall_overlap
            True,  # use_padding
            dense_token_type,
            moe_statics,
        )
        ...
        # assert self.gate.config.sequence_parallel
        world_size = self.gate.config.moe_world_size
        if isinstance(self.gate.config.moe_num_experts, (list, tuple)):
            self.global_num_experts = sum(self.gate.config.moe_num_experts)
        else:
            self.global_num_experts = self.gate.config.moe_num_experts
        self.local_num_experts = len(experts)
        self.token_dispatcher = MoEFlexTokenDispatcher(
            self.local_num_experts, self.global_num_experts, self.config.moe_group
        )
        self.k = k
        # 全局 gate gather
        self.send_rank = None
        ...
        self.dense_experts = dense_experts
        self.capacity_tensor = None
        logger.info("uisng DeepEPMOELayerMultiModal")
        self.two = paddle.to_tensor(2, dtype=paddle.float32)
        self.zero = paddle.to_tensor(0, dtype=paddle.float32)
    ...
    def forward(
        ...
        ), "use_ep_comm_overlap is not supported yet in DeepEPMOELayerMultiModal"
        ...
        if self.config.deepep_fine_grained:
            ...
            # global dispatch
            with profile("moe_dispatch"):
                dispatched_output, dispatched_indices, dispatched_probs = (
                    self.token_dispatcher._comm_manager.dispatch(
                        input, local_expert_id, local_combine_weights
                    )
                )
            ...
            with profile("forward_experts"):
                output_tokens = self.fine_grained_forward_experts(
                    dispatched_output, dispatched_probs, dispatched_indices, self.k
                )

            # global combine
            with profile("moe_combine"):
                combined_output = self.token_dispatcher._comm_manager.combine(
                    output_tokens
                )
        else:
            with profile("moe_dispatch"):
                (
                    ...
                    input, local_expert_id, local_combine_weights, self.k
                )
            ...
            # global -> local
            with profile("moe_combine"):
                combined_output = self.token_dispatcher.token_unpermutation(
                    expert_out,
                    token_permuted_indices,
                    prob_permuted_indices,
                    dispatched_probs,
        ...
        with profile("dense-expert"):
            if use_dense_expert:
                combined_output += self.dense_experts(input)
        ...
        return combined_output, local_combine_weights, router_loss2, gate_logits_lm
        # allgather
    ...
    def fused_gate_and_dispatch(self, input, token_type_ids, global_dense_expert_mask):
        ...
        expert_id = expert_id.cast("int64")
        expert_id.stop_gradient = True
        num_experts = (
            sum(self.config.moe_num_experts)
            if isinstance(self.config.moe_num_experts, (tuple, list))
        ...
        masked_combine_weights = paddle.zeros([seqlen, num_experts]).put_along_axis(
            expert_id, combine_weights, axis=-1
        )
        router_mask = (masked_combine_weights != 0.0).cast(masked_combine_weights.dtype)
        expert_num_local = paddle.sum(router_mask, axis=0).cast("int64")
        expert_num_global = expert_num_local.clone()
        dist.all_reduce(expert_num_global, group=self.config.moe_group)
        expert_num_global.stop_gradient = True
