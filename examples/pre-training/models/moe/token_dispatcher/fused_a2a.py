...
from models.fp8_linear import kitchen_quant
...
def barrier_ep():
    ...
    if hasattr(hcg, "get_expert_parallel_group"):
        ...
        warnings.warn(
            "No get_expert_parallel_group found, please check your environment."
        )
...
if HAVE_DEEP_EP:
    ...
    def fused_dispatch(
        ...
        if inner_layer_overlap_handle is None:
            ...
            for arg in inner_layer_overlap_handle["fn_args"]:
                if arg is None:
                    continue
                assert isinstance(
                    arg, paddle.Tensor
                ), f"fn_args in inner_layer_overlap_handle must be a Tensor, but got {type(arg)}"

            recv_x, recv_token_probs, states, fp8_dispatched_handle, *fn_out = (
                FusedDispatchAsync.apply(
                    x.contiguous(),
                    token_indices,
                    token_probs,
                    num_experts,
