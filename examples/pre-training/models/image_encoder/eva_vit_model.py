...
def drop_path(
    ...
    bern_0 = (
        paddle.to_tensor([keep_prob], dtype=paddle.float32)
        if not isinstance(keep_prob, paddle.Tensor)
        else keep_prob
    )
    random_tensor = paddle.assign(
        paddle.bernoulli(
            paddle.broadcast_to(bern_0, paddle.empty(shape=shape, dtype=x.dtype).shape)
        ),
...
class RMSNorm(paddle.nn.Layer):
    ...
    def __init__(self, hidden_size, fuse_rms_norm=True, eps=1e-6):
        """
...
class DropPath(paddle.nn.Layer):
    ...
    def __init__(self, drop_prob=None, tensor_parallel_degree=-1):
...
class Mlp(paddle.nn.Layer):
    ...
    def __init__(
        self, config, act_layer=paddle.nn.GELU, norm_layer=paddle.nn.LayerNorm
    ):
        super().__init__()
...
class Attention(paddle.nn.Layer):
    ...
    def __init__(
        self, config, window_size=None, rope=None, norm_layer=paddle.nn.LayerNorm
    ):
        super().__init__()
        self.config = config
        dim = config.width
...
class PatchEmbedForPatchified(paddle.nn.Layer):
    """Image to Patch Embedding"""
    ...
    def __init__(self, config):
        ...
        self.proj = paddle.nn.Linear(
            config.in_chans * patch_size[0] * patch_size[1], config.width
        )
...
class EVAVisionTransformer(EVAVisionTransformerPretrainedModel):
    ...
    def __init__(self, config: EVAVisionTransformerConfig):
        ...
        if config.use_rms_norm:
            ...
            norm_layer = partial(RMSNorm, fuse_rms_norm=config.fuse_rms_norm, eps=1e-6)
        else:
            norm_layer = (
        ...
        if config.attentional_pool:
            ...
            self.norm = (
                paddle.nn.Identity() if use_mean_pooling else norm_layer(output_dim)
            )
            ...
            if config.need_head:
                if config.tensor_parallel_degree > 1:
                    ...
                    self.head = (
                        paddle.nn.Linear(output_dim, output_dim)
                        if output_dim > 0
                        else paddle.nn.Identity()
                    )
        else:
            ...
            if config.need_head:
                if config.tensor_parallel_degree > 1:
                    ...
                    self.head = (
                        paddle.nn.Linear(width, output_dim, bias_attr=config.head_bias)
                        if output_dim > 0
                        else paddle.nn.Identity()
                    )
...
class AttentionalPooler(paddle.nn.Layer):
    ...
    def __init__(self, config, norm_layer: Callable = LayerNorm):
        ...
        self.attn = MultiHeadAttention(
            d_model, config.attn_pooler_heads, kdim=context_dim, vdim=context_dim
        )
