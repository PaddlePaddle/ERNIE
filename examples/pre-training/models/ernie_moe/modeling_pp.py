...
from models.ernie_moe.configuration import ErnieMoEConfig
from models.sequence_parallel_utils import (
    ScatterOp,
...
class ErnieMoEForCausalLMPipe(PipelinePretrainedModel, PipelineLayer):
    ...
    def _prepare_pipeline_inputs_func(cls, data):
        ...
        #     for k in data.keys()
        # }
        # data_id = data.pop('data_id', None) #no use
        # src_id = data.pop('src_id', None) #no use
        inputs = tuple(
            [
                [d[k] for d in data]
                for k in [
    ...
    def __init__(
        ...
        # self.rename_model_params(set_name_for_eblite)
...
# HACK op names, break everything
def set_name_for_eblite(config, layers):
    """set name for eblite"""
    _layers = layers
    ...
    tensor_parallel_rank = max(tp_group.rank, 0)

    # 赋予静态图名字：
    for layer in _layers:
        if isinstance(layer, ErnieEmbeddingPipe):
            layer.embed_tokens.weight.name = (
                f"word_embedding_expanded_{tensor_parallel_rank}.w_0"
            )
        elif isinstance(layer, ErnieDecoderLayerPipe):
            layer_idx = layer.layer_idx
            if config.fuse_attn_ffn:
                if config.use_rms_qkv_recompute:
                    layer.self_attn.fused_rms_norm_linear.linear_weight.name = f"encoder_layer_{layer_idx}_multi_head_att_qkv_fc_{tensor_parallel_rank}.w_0"
                else:
                    layer.self_attn.qkv_proj.weight.name = f"encoder_layer_{layer_idx}_multi_head_att_qkv_fc_{tensor_parallel_rank}.w_0"
                # layer.mlp.up_gate_proj.weight.name = \
                #       f'encoder_layer_{layer_idx}_ffn_fc_0_{tensor_parallel_rank}.w_0'  # 静态图名字来自 EB35代码
                if config.use_bias:
                    layer.self_attn.qkv_proj.bias.name = f"encoder_layer_{layer_idx}_multi_head_att_qkv_fc_{tensor_parallel_rank}.b_0"
                    # layer.mlp.up_gate_proj.bias.name = \
                    #       f'encoder_layer_{layer_idx}_ffn_fc_0_{tensor_parallel_rank}.b_0'  # 静态图名字来自 EB35代码

            else:
                layer.self_attn.q_proj.weight.name = f"encoder_layer_{layer_idx}_multi_head_att_query_fc_{tensor_parallel_rank}.w_0"
                layer.self_attn.k_proj.weight.name = f"encoder_layer_{layer_idx}_multi_head_att_key_fc_{tensor_parallel_rank}.w_0"
                layer.self_attn.v_proj.weight.name = f"encoder_layer_{layer_idx}_multi_head_att_value_fc_{tensor_parallel_rank}.w_0"
                # layer.mlp.gate_proj.weight.name = \
                #       f'encoder_layer_{layer_idx}_ffn_fc_0_up_{tensor_parallel_rank}.w_0'
                # layer.mlp.up_proj.weight.name = \
                #       f'encoder_layer_{layer_idx}_ffn_fc_0_gate_{tensor_parallel_rank}.w_0'
                if config.use_bias:
                    layer.self_attn.q_proj.bias.name = f"encoder_layer_{layer_idx}_multi_head_att_query_fc_{tensor_parallel_rank}.b_0"
                    layer.self_attn.k_proj.bias.name = f"encoder_layer_{layer_idx}_multi_head_att_key_fc_{tensor_parallel_rank}.b_0"
                    layer.self_attn.v_proj.bias.name = f"encoder_layer_{layer_idx}_multi_head_att_value_fc_{tensor_parallel_rank}.b_0"
                    # layer.mlp.gate_proj.bias.name = \
                    #       f'encoder_layer_{layer_idx}_ffn_fc_0_up_{tensor_parallel_rank}.b_0'
                    # layer.mlp.up_proj.bias.name = \
                    #       f'encoder_layer_{layer_idx}_ffn_fc_0_gate_{tensor_parallel_rank}.b_0'

            if config.use_linear_residual_norm_recompute:
                layer.fused_linear_add_norm.linear_weight.name = f"encoder_layer_{layer_idx}_multi_head_att_output_fc_{tensor_parallel_rank}.w_0"
            else:
                layer.self_attn.o_proj.weight.name = f"encoder_layer_{layer_idx}_multi_head_att_output_fc_{tensor_parallel_rank}.w_0"
            # layer.mlp.down_proj.weight.name = f'encoder_layer_{layer_idx}_ffn_fc_1_{tensor_parallel_rank}.w_0'
            if config.use_rms_qkv_recompute:
                layer.fused_linear_add_norm.rms_norm_weight.name = (
                    f"encoder_layer_{layer_idx}_pre_att_layer_norm_scale_0.w_0"
                )
            else:
                layer.input_layernorm.weight.name = (
                    f"encoder_layer_{layer_idx}_pre_att_layer_norm_scale_0.w_0"
                )
            layer.post_attention_layernorm.weight.name = (
                f"encoder_layer_{layer_idx}_pre_ffn_layer_norm_scale_0.w_0"
            )

            if config.use_bias:
                layer.self_attn.o_proj.bias.name = f"encoder_layer_{layer_idx}_multi_head_att_output_fc_{tensor_parallel_rank}.b_0"
                # layer.mlp.down_proj.bias.name = \
                #       f'encoder_layer_{layer_idx}_ffn_fc_1_{tensor_parallel_rank}.b_0'

            if hasattr(layer.mlp, "experts"):
                for exp_idx, expert in enumerate(layer.mlp.experts):
                    if config.fuse_attn_ffn:
                        expert.up_gate_proj.weight.name = f"encoder_layer_{layer_idx}_expert_{exp_idx}_ffn_fc_0_{tensor_parallel_rank}.w_0"
                        if config.use_bias:
                            expert.up_gate_proj.bias.name = f"encoder_layer_{layer_idx}_expert_{exp_idx}_ffn_fc_0_{tensor_parallel_rank}.b_0"
                    else:
                        raise ValueError("not supperted yet")
                    expert.down_proj.weight.name = f"encoder_layer_{layer_idx}_expert_{exp_idx}_ffn_fc_1_{tensor_parallel_rank}.w_0"
                    if config.use_bias:
                        expert.down_proj.bias.name = f"encoder_layer_{layer_idx}_expert_{exp_idx}_ffn_fc_1_{tensor_parallel_rank}.b_0"
                # mlp.gate 不需要改名字
            else:
                if config.fuse_attn_ffn:
                    layer.mlp.up_gate_proj.weight.name = f"encoder_layer_{layer_idx}_ffn_fc_0_{tensor_parallel_rank}.w_0"  # 静态图名字来自 EB35代码
                    if config.use_bias:
                        layer.mlp.up_gate_proj.bias.name = f"encoder_layer_{layer_idx}_ffn_fc_0_{tensor_parallel_rank}.b_0"

                else:
                    layer.mlp.gate_proj.weight.name = f"encoder_layer_{layer_idx}_ffn_fc_0_up_{tensor_parallel_rank}.w_0"
                    layer.mlp.up_proj.weight.name = f"encoder_layer_{layer_idx}_ffn_fc_0_gate_{tensor_parallel_rank}.w_0"

                    if config.use_bias:
                        layer.mlp.gate_proj.bias.name = f"encoder_layer_{layer_idx}_ffn_fc_0_up_{tensor_parallel_rank}.b_0"
                        layer.mlp.up_proj.bias.name = f"encoder_layer_{layer_idx}_ffn_fc_0_gate_{tensor_parallel_rank}.b_0"
                layer.mlp.down_proj.weight.name = (
                    f"encoder_layer_{layer_idx}_ffn_fc_1_{tensor_parallel_rank}.w_0"
                )
                if config.use_bias:
                    layer.mlp.down_proj.bias.name = (
                        f"encoder_layer_{layer_idx}_ffn_fc_1_{tensor_parallel_rank}.b_0"
            ...
            if not config.use_rmsnorm:
                layer.input_layernorm.bias.name = (
                    f"encoder_layer_{layer_idx}_pre_att_layer_norm_bias_0.b_0"
                )
                layer.post_attention_layernorm.bias.name = (
                    f"encoder_layer_{layer_idx}_pre_ffn_layer_norm_bias_0.b_0"
                )

        elif isinstance(layer, LayerNormPipe):
            layer.weight.name = "encoder_post_encoder_layer_norm_scale_0.w_0"
            layer.bias.name = "encoder_post_encoder_layer_norm_bias_0.b_0"

        elif isinstance(layer, RMSNormPipe):
            layer.weight.name = "encoder_post_encoder_layer_norm_scale_0.w_0"
        elif isinstance(layer, ErnieMoELMHead):
            layer.weight.name = f"server_nlg_mask_lm_out_fc_{tensor_parallel_rank}.w_0"
            if config.use_bias:
                layer.bias.name = (
                    f"server_nlg_mask_lm_out_fc_{tensor_parallel_rank}.b_0"
                )
