...
class ErniemmConfig(PretrainedConfig):

    model_type = "erniemm"
    is_composition = True

    def __init__(
        self, vision_config=None, text_config=None, im_patch_id="1004", **kwargs
    ):  # [unused999]$
        ...
        if text_config is None:
            text_config = {}
            logger.info(
                "text_config is None. Initializing the text config with default values (`ErnieConfig`)."
            )

        if "clip" in vision_config["model_type"]:
            self.vision_config = CLIPVisionConfig(**vision_config)
        else:
            raise NotImplementedError

        text_model_type = (
            text_config["model_type"] if "model_type" in text_config else "ernie"
        )
        if text_model_type == "ernie":
            self.text_config = ErnieConfig(**text_config)
        else:
            raise NotImplementedError
