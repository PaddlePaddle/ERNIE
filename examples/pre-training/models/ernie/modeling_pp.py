...
class PipelinePretrainedModel(PretrainedModel):
    ...
    def sharded_state_dict(self, *args, **kwargs):
        ...
        logger.info("====> debug  PipelinePretrainedModel sharded state dict")
        for k,v in self._pp_to_single_mapping.items():
            logger.info(f"==> {k}:{v}")
        ...
        for k, v in sharded_state_dict.items():
            ...
            # else:
            #     if k.startswith("ernie.embed.embed_tokens"):
            #         new_key = k.replace("ernie.embed.embed_tokens", "ernie.embed_tokens")
