...
# Communication timeout settings (recommended to keep)
export PADDLE_PG_TIMEOUT=150000

# PP/TP initialization optimization
export FLAGS_pipeline_nccl_comm_init_option=1

# Disable CUDNN Fast Algorithm (suitable for new GPUs like H100)
export PADDLE_DISABLE_CUDNN_FA=1

# Clean up shared memory cache to prevent conflicts
find /dev/shm/ -type f -name "paddle_*" -delete
...
python -m paddle.distributed.launch \
    ...
    > ${train_log_name}.log 2>${train_log_name}_err.log &

    tail -f ${train_log_name}.log
