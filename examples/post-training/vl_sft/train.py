...
class ChatSFTArguments(PreTrainingArguments):
    ...
    eval_task_config: str = field(default=None, metadata={"help": "Path to eval_task_config."})
    ...
    add_header_eos_token: bool = field(
        default=False, metadata={"help": "use <header_start> <header_end> </s> tokens segment system info"}
    )
    min_shot: int = field(default=2, metadata={"help": "min shot"})
    max_shot: int = field(default=8, metadata={"help": "max shot"})

    all_use_skip_align: bool = field(default=False, metadata={"help": "use skip align strategy"})
    skip_align_s_policy: str = field(default="default", metadata={"help": "skip align strategy"})
    skip_align_target_length: int = field(default=100000, metadata={"help": "skip align max length"})
    skip_align_sub_sample_prob: float = field(default=0.5, metadata={"help": "skip align with prob"})
    skip_align_method: str = field(default="skip_outter", metadata={"help": "skip align method"})

    follow_deepseek: bool = field(default=True, metadata={"help": "use the new concat of deepseek"})

    sampling_wo_replacement_data_resuming: Optional[bool] = field(
        default=True, metadata={"help": "save and load state of SFT data, support resuming without replacement"}
    )
    ...
    offload_optim: Optional[bool] = field(default=True, metadata={"help": "use offload_optim"})
...
class ModelArguments:
    add_tail_layers: bool = field(default=False, metadata={"help": "Whether to add tail layers"})
    attention_probs_dropout_prob: float = field(default=0.0, metadata={"help": "Attention probability dropout rate"})
    bos_token_id: int = field(default=0, metadata={"help": "Beginning of sentence token ID"})
    cachekv_quant: bool = field(default=False, metadata={"help": "Whether to quantize cache key/value"})
    compression_ratio: float = field(default=1.0, metadata={"help": "Compression ratio"})
    dpo_config: Optional[Dict] = field(default=None, metadata={"help": "DPO configuration"})
    enable_delay_scale_loss: bool = field(default=True, metadata={"help": "Whether to enable delay scale loss"})
    ...
    fuse_gate_detach_matmul: bool = field(default=True, metadata={"help": "Whether to fuse gate detach matmul"})
    fuse_linear: bool = field(default=True, metadata={"help": "Whether to fuse linear layers"})
    fuse_ln: bool = field(default=False, metadata={"help": "Whether to fuse layer normalization"})
    fuse_rms_norm: bool = field(default=False, metadata={"help": "Whether to fuse RMS normalization"})
    fuse_rope: bool = field(default=False, metadata={"help": "Whether to fuse RoPE"})
    fuse_softmax_mask: bool = field(default=False, metadata={"help": "Whether to fuse softmax mask"})
    fuse_swiglu: bool = field(default=True, metadata={"help": "Whether to fuse SwiGLU"})
    global_aux_loss: bool = field(default=False, metadata={"help": "Whether to use global auxiliary loss"})
    head_dim: Optional[int] = field(default=None, metadata={"help": "Head dimension"})
    hidden_act: str = field(default="silu", metadata={"help": "Hidden activation function"})
    ...
    moe_all_to_all_dropout: float = field(default=0.0, metadata={"help": "MoE all-to-all dropout rate"})
    moe_aux_loss_lambda: float = field(default=0.0, metadata={"help": "MoE auxiliary loss lambda"})
    moe_capacity: List[int] = field(default_factory=lambda: [128, 128, 128], metadata={"help": "MoE capacity"})
    moe_dense_experts_token_type_id: int = field(default=3, metadata={"help": "MoE dense experts token type ID"})
    moe_fuse_experts: bool = field(default=False, metadata={"help": "Whether to fuse MoE experts"})
    moe_gate: str = field(default="top2_fused", metadata={"help": "MoE gate type"})
    moe_gate_act: str = field(default="softmax", metadata={"help": "MoE gate activation"})
    moe_group_experts: bool = field(default=False, metadata={"help": "Whether to group MoE experts"})
    moe_group_orthogonal_loss: bool = field(default=True, metadata={"help": "Whether to use MoE group orthogonal loss"})
    moe_intermediate_size: List[int] = field(default_factory=lambda: [1536, 512], metadata={"help": "MoE intermediate sizes"})
    moe_k: int = field(default=6, metadata={"help": "MoE top-k value"})
    moe_layer_end_index: List[int] = field(default_factory=lambda: [29, 28], metadata={"help": "MoE layer end indices"})
    moe_layer_feed_fake_token: bool = field(default=False, metadata={"help": "Whether MoE layers feed fake tokens"})
    moe_layer_interval: int = field(default=1, metadata={"help": "MoE layer interval"})
    moe_layer_start_index: List[int] = field(default_factory=lambda: [1, 1], metadata={"help": "MoE layer start indices"})
    moe_multimodal_dispatch_use_allgather: str = field(default="v2-alltoall-unpad-text", metadata={"help": "MoE multimodal dispatch method"})
    moe_norm_gate_logits: bool = field(default=True, metadata={"help": "Whether to normalize MoE gate logits"})
    moe_num_experts: List[int] = field(default_factory=lambda: [64, 64], metadata={"help": "Number of MoE experts"})
    moe_num_shared_experts: int = field(default=2, metadata={"help": "Number of shared MoE experts"})
    moe_orthogonal_loss_lambda: float = field(default=0.0, metadata={"help": "MoE orthogonal loss lambda"})
    moe_reverse_token_drop: bool = field(default=False, metadata={"help": "Whether to reverse MoE token drop"})
    moe_use_aux_free: bool = field(default=True, metadata={"help": "Whether MoE uses auxiliary free"})
    moe_use_hard_gate: bool = field(default=True, metadata={"help": "Whether MoE uses hard gate"})
    moe_use_size_all2all: bool = field(default=False, metadata={"help": "Whether MoE uses size all-to-all"})
    moe_use_token_type_bias: bool = field(default=False, metadata={"help": "Whether MoE uses token type bias"})
    moe_z_loss_lambda: float = field(default=0.0, metadata={"help": "MoE z loss lambda"})
    multi_token_pred_lambda: float = field(default=0.3, metadata={"help": "Multi-token prediction lambda"})
    num_acc_steps: int = field(default=4, metadata={"help": "Number of accumulation steps"})
    num_attention_heads: int = field(default=20, metadata={"help": "Number of attention heads"})
    num_hidden_layers: int = field(default=28, metadata={"help": "Number of hidden layers"})
    num_key_value_heads: int = field(default=4, metadata={"help": "Number of key/value heads"})
    num_nextn_predict_layers: int = field(default=0, metadata={"help": "Number of next-n prediction layers"})
    paddleformers_version: Optional[str] = field(default=None, metadata={"help": "PaddleFormers version"})
    pp_seg_method: str = field(default="layer:Ernie4_5_DecoderLayer|EmptyLayer", metadata={"help": "Pipeline parallel segmentation method"})
    recompute_granularity: str = field(default="core_attn", metadata={"help": "Recompute granularity"})
    resampler_fuse_rms_norm: bool = field(default=False, metadata={"help": "Whether to fuse RMS norm in resampler"})
    rms_norm_eps: float = field(default=1e-05, metadata={"help": "RMS normalization epsilon"})
    rope_theta: int = field(default=500000, metadata={"help": "RoPE theta value"})
    sinkhorn_2gate: bool = field(default=True, metadata={"help": "Whether to use Sinkhorn 2-gate"})
    sinkhorn_temp: float = field(default=0.03, metadata={"help": "Sinkhorn temperature"})
    skip_recompute_ops: Dict = field(default_factory=dict, metadata={"help": "Skip recompute operations"})
    ...
    use_recompute_moe: bool = field(default=False, metadata={"help": "Whether to recompute MoE"})
    use_recompute_mtp: bool = field(default=False, metadata={"help": "Whether to recompute MTP"})
    use_recompute_resampler: bool = field(default=False, metadata={"help": "Whether to recompute resampler"})
    use_rmsnorm: bool = field(default=True, metadata={"help": "Whether to use RMS normalization"})
    use_sparse_flash_attn: bool = field(default=True, metadata={"help": "Whether to use sparse flash attention"})
    use_sparse_head_and_loss_fn: bool = field(default=False, metadata={"help": "Whether to use sparse head and loss function"})
    use_temporal_conv: bool = field(default=True, metadata={"help": "Whether to use temporal convolution"})
    use_var_len_flash_attn: bool = field(default=False, metadata={"help": "Whether to use variable length flash attention"})
    ...
    weight_share_add_bias: bool = field(default=True, metadata={"help": "Whether to share weight and add bias"})
...
def get_resume_checkpoint_path(config):
    ...
    if (
        ...
        if pdc_init_step is None:
            logger.info("launching training process from scratch with no resume step defined.")
            return None
...
def main():
    ...
    parser = PdArgumentParser((ChatSFTArguments, ModelArguments, *End2EndProcessorArguments))
    ...
    args.enable_delay_scale_loss = "enable_delay_scale_loss" in args.pipeline_parallel_config

    if args.modality_ratio is not None:
        args.modality_ratio = ''.join(args.modality_ratio).replace('[', '').replace(']', '')
        ...
        args.modality_ratio = [i / sum(args.modality_ratio) for i in args.modality_ratio]
    ...
    if args.do_train:
        ...
        if args.need_data and args.is_train_text:
            ...
            config_dataset_text = {
                ...
                "load_optimizer_and_scheduler": False if args.resume_from_ptx_model else True,
            ...
            for task_in_reader, task in zip(text_sft_train_reader.task_group, train_task_group_text):
